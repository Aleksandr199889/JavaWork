...
/*
В этом примере класс ToyStore содержит внутренний класс Toy, 
представляющий игрушку с полями id, name и frequency. 
Интерфейс Comparable реализован для сравнения игрушек по их частоте выпадения.
В конструкторе ToyStore создается и инициализируется очередь игрушек toyQueue 
с использованием PriorityQueue, которая будет автоматически упорядочивать игрушки
 по возрастанию частоты выпадения.
Метод parseToy используется для разбора строки, представляющей игрушку, и создания объекта Toy.
Метод addToy позволяет добавить новую игрушку в очередь.
Метод getToyQueue возвращает ссылку на внутреннюю очередь игрушек toyQueue.
В методе main приведен пример использования класса ToyStore, где создается экземпляр ToyStore с тремя игрушками, 
а затем добавляется еще одна игрушка. Затем игрушки из очереди toyQueue извлекаются и выводятся на консоль в порядке увеличения частоты выпадения.
*/